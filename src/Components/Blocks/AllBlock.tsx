...
import {
  ...
  selectBlocksLoadingInFrames,
  selectBlocksPerPage,
  selectCurrentPage,
  selectHomePageBlocks,
  setBlocksInFramesLoading,
  setCurrentPage,
  setNewTrxCount,
...
const AllBlock: React.FC<AllBlockProps> = ({ showTrx }) => {
  ...
  const fetchLatestFrame = () => {
    blocks;
    const startBlock =
      Number(blocks[0].number) - (currentPage - 1) * blocksPerPage;
    dispatch(
      fetchBlocksInFrame({
        rpcUrl,
  ...
  const renderPageButton = (pageNumber: number) => (
    <Button
      ...
      style={{
        ...
        border:
          pageNumber === currentPage ? "1px solid #007bff" : "1px solid #ccc",
      }}
    >
      {pageNumber}
    </Button>
      ...
      >
        {showTrx &&
          blocks.map((block, idx) => (
            <SingleBlockInfo
              key={`trx-${block.hash ?? idx}`} // fallback to index if no hash
              block={block}
        ...
        {homePageBlocks.map((block, idx) => (
          <SingleBlockInfo
            key={`home-${block.hash ?? idx}`} // fallback key
            block={block}
            loading={allBlocksLoading}
            isNew={block.hash === latestBlockHash}
            showTrx={showTrx}
